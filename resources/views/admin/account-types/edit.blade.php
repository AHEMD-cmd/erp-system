                    ...
                    <form action="{{ route('admin.uoms.update', $uom->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" id="name"
                                class="form-control @error('name') is-invalid @enderror" placeholder="أدخل اسم الوحدة"
                                value="{{ old('name', $uom->name) }}" required>
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <select name="is_master" id="is_master"
                                ...
                                <option value="1" {{ old('is_master', $uom->is_master) == '1' ? 'selected' : '' }}>وحدة اب</option>
                                <option value="0" {{ old('is_master', $uom->is_master) == '0' ? 'selected' : '' }}>وحدة تجزئة</option>
                            </select>
                            @error('is_master')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select name="active" id="active" class="form-control @error('active') is-invalid @enderror"
                                required>
                                <option value="1" {{ old('active', $uom->active) == '1' ? 'selected' : '' }}>مفعل</option>
                                <option value="0" {{ old('active', $uom->active) == '0' ? 'selected' : '' }}>معطل</option>
                            </select>
                            @error('active')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
